...
ta_oa_growth <- ta_oa_inst |>
  filter(ta_active == TRUE, !is.na(cc), issn_l != "0027-8424", cr_year != "2023") |>
  group_by(cr_year) |>
  summarise(ta_oa = n_distinct(doi))
hybrid_oa_growth <- hoaddata::cc_articles |>
  filter(issn_l != "0027-8424", cr_year != "2023") |>
  group_by(cr_year) |>
...
n_total <- hoaddata::jn_ind |>
  filter(issn_l != "0027-8424", cr_year != "2023", !is.na(cc)) |>
  distinct(issn_l, cr_year, jn_all) |>
  pull(jn_all) |>
  sum()
...
During this period, `r n_oa` out of `r n_total` articles in these journals became open access, representing a six-year hybrid open access uptake rate of `r n_oa / n_total * 100`%.
...
box_ta_prop <- summary_all |>
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)),
                     labels = scales::percent) +
  coord_cartesian(ylim = c(0,0.65)) +
  scale_fill_manual("Hybrid OA", values = c(prop_no_ta = "#b3b3b3a0", prop_ta_oa = "#56B4E9"), labels = c(prop_no_ta = "Other", prop_ta_oa = "Agreement"), guide = FALSE) +
  theme_minimal() +
