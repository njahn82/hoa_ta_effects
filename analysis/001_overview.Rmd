...
country_bar <- country_scatter_df |>
  group_by(cr_year, cat) |>
  summarise(all = sum(all), oa = sum(oa), ta_oa = sum(ta_oa)) |>
  mutate(closed = all - oa) |>
  pivot_longer(cols = c(oa, ta_oa, closed), names_to = "oa_type") |>
  ggplot(aes(gsub("^20", "'", as.character(cr_year)), value, group = oa_type, fill = oa_type)) +
  geom_area() +
  facet_wrap(~ cat) +
  scale_fill_manual(NULL, values = c(oa = "#b3b3b3a0", ta_oa = "#56B4E9", closed = "#e98b56"),
    labels = c(oa = "OA", ta_oa = "Agreement", closed = "Closed")) +
  theme_minimal() +
  scale_y_continuous(labels =  scales::number_format(big.mark = ",")) +
  theme_minimal() +
