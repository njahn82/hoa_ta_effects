...
oa_ta_plot <- ggplot(oa_ta_plot_df, aes(cr_year, value / n, fill = type, group = type)) +
  geom_area(color = "white",
            alpha = 0.8,
            linewidth = 0.3) +
  scale_fill_manual(NULL, values = c(no_ta = "#b3b3b3", ta_oa = "#56B4E9"), labels = c(no_ta = "Other", ta_oa = "Agreement"),  guide = guide_legend(reverse = TRUE)) +
...
publisher_prop <- ggplot(publisher_summary_df, aes(gsub("^20", "'", as.character(cr_year)), prop, fill = type, group = type)) +
  ...
  scale_fill_manual("Hybrid OA", values = c(no_ta = "#b3b3b3", ta_oa = "#56B4E9"), labels = c(no_ta = "Other", ta_oa = "Agreement"),  guide = guide_legend(reverse = TRUE)) +
...
scale_fill_manual(
    "Hybrid OA",
    values = c(no_ta = "#b3b3b3", ta_oa = "#56B4E9"),
...
top_20_plot <- top_20_df |>
  ...
  ggplot(aes(reorder(as.character(country_code),  value), value / sum(value), fill = type)) +
    geom_bar(stat = "identity", alpha = 0.8, color = "#000000", linewidth = 0.2) +
   scale_fill_manual(
    NULL,
    values = c(no_ta = "#b3b3b3", ta_oa = "#56B4E9"),
    labels = c(no_ta = "Other", ta_oa = "Agreement"),
...
country_scatter_22 <- country_scatter_df |>
  ...
  scale_fill_manual(NULL, values = c("OECD" = "#3fc1c9", "BRICS" = "#fc5185", "Other" = "#e1e1e1")) +
  geom_smooth(aes(all, oa), method=lm) +
  scale_size("TA OA", guide = "none") +
  ggrepel::geom_text_repel(
    data = filter(country_scatter_df, cr_year == 2022, cat %in% c("OECD", "BRICS"), ta_oa > 5000 | country_code %in% c("CN", "IN")),
...
country_bar <- ggplot(oecd_stats_df, aes(gsub("^20", "'", as.character(cr_year)), value, group = oa_type, fill = oa_type, color = oa_type)) +
  geom_area(color = "#000000",
            alpha = 0.8,
            linewidth = 0.2) +
  scale_color_manual(values = colorspace::darken(c(oa_other = "#b3b3b3", ta_oa = "#56B4E9", closed = "#e1e1e1"), 0.3)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), labels =  scales::number_format(big.mark = ",")) +
  facet_wrap(~ cat) +
  scale_fill_manual("Hybrid OA", values = c(oa_other = "#b3b3b3", ta_oa = "#56B4E9", closed = "#e1e1e1"),
