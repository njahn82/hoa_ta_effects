...
Analysing hybrid open access across publishers between 2018 and 2022 reveals a large concentration on a few publishers in terms of publication volume. 
While transformative agreements could be attributed to `r length(unique(publisher_plot_df$esac_publisher))` distinct publishers, the three publishers Elsevier, Springer Nature and Wiley accounted for xxx percent of articles published. Together, they published xxx articles, representing a share of xx percent. xxx were published through transformative agreements. 
The remainder consists of

```{r publisher_summary_table}
publisher_article_total_table <- publisher_summary_df |>
  distinct(publisher, jn_all) |>
  group_by(publisher) |>
  summarise(publisher_total = sum(jn_all)) |>
  mutate(perc = publisher_total / sum(publisher_total),
...
publisher_article_ta_oa_table <- publisher_summary_df |>
  ...
  mutate(ta_oa_perc = ta_oa_total / sum(ta_oa_total),
...
kbl(publisher_league_tbl) %>%
  kable_classic() %>%
  add_header_above(c("Publisher" = 1, "Group 1" = 3, "Group 2" = 3, "Group 3" = 3))
```

Figure 2 presents the growth of hybrid open access across publishers by year, highlighting the proportion of open articles published under transformative agreements. As can be seen from Figure 2A and 2C 
