...
After identification of hybrid journals, Crossref served as the primary data source for article-level metadata including Creative Commons (CC) license information to determine open access availability on publisher websites.
To identify open access articles published through transformative agreements, first author affiliations from OpenAlex were subsequently linked to eligible institutions according to the transformative agreement data.
...
A limitation of using the Journal Checker Tool to study the development of transformative agreements over time is that it only provided data about active transformative agreements.
As a result, expired transformative agreements were constantly removed from the publicly available data dump. 
...
This ensured that transformative agreements, which ended from 2021 onwards, were included.
...
After identifying hybrid journals included in transformative agreements, article metadata was retrieved from Crossref<!. <!-- and mapped to OpenAlex for obtaining author metadata including affiliations. --> November 2023 database snapshot for the five-year period 2018 to 2022. 
...
Comparing Crossref license coverage with OpenAlex, which re-uses open access evidence from Unpaywall, not all publisher provide such license information a widely used open access discovery service that also parses journal websites for open content licenses [@Piwowar_2018], demonstrates the ongoing challenges to identify open access articles in hybrid journals [@Mart_n_Mart_n_2018; @Jahn_2021].
...
Here, affiliations were retrieved from OpenAlex.
...
`r cr_stats |> filter(src == "gold") |> pull(n_articles) |> sum() / cr_stats |> pull(n_articles) |> sum() * 100`% of total article volume.
...
Following @Marwick_2018, hoaddata contains not only the datasets used in the data analysis. It also includes codeused to compile the data by connecting it the cloud-based Google Big Query data warehouse, where the big scholarly data from Crossref, OpenAlex and Unpaywall were imported.
