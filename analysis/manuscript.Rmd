...
In total, the compiled data set consists of `r hoaddata::jn_ind |> filter(!cr_year %in% c("2017", "2023")) |> distinct(issn_l, cr_year, jn_all) |> pull(jn_all) |> sum()` articles published in `r hoaddata::jn_ind |> filter(!cr_year %in% c("2017", "2023")) |> distinct(issn_l) |> nrow()` hybrid journals between 2018 and 2022. 
...
Data gathering started with obtaining journals included in transformative agreements using the cOAlition S’ Journal Checker Tool publicly available Transformative Agreement Data dump.[^tadump] 
...
Because the Journal Checker Tool only only provides data about active transformative agreements, removing data representing expired transformative agreements, four different snapshots were used for this analysis: self-archived version from July 2021, July 2022, and May 2023, as well as the most current dump downloaded on 11 December 2023. 
...
The Transformative Agreement Data dumps linked agreements to journals represented by journal names and ISSN. 
...
After identifying hybrid journals included in transformative agreements, article metadata was retrieved from Crossref and mapped to OpenAlex to obtain affiliation metadata. 
The Crossref November 2023 database snapshot was used to determine the journals’ yearly publication volume for the 5-year period 2018 to 2022. 
Because Crossref metadata did not distinguish between research articles and other types of journal content, only included articles published in regular issues indicated by non-numeric pagination were included.
...
Open access articles in hybrid journals were identified through Creative Commons (CC) license information in Crossref metadata and mapped them to the different CC variants. 
...
The biggest differences concerned articles published between 2018 and 2020, and among some publishers that presumably did not provide CC license information to Crossref including AIP Publishing, American Physiological Society and Emerald.
Crossref license metadata was more complete with regard to articles from the publisher Wiley.
During the data collection, inconsistent open access status information in previous OpenAlex version was observed [@jahn2023], which might explain this discrepancy.

Next, the number and proportion of open access articles per journal was calculated. 
...
Affiliation data is a crucial data point for the planning and evaluation of transformative agreements [@Schimmer_2015; @Geschuhn_2017]. 
Author affiliations were retrieved from OpenAlex.
Generally, corresponding authors are responsible to arrange open access publication [@Borrego_2020].
However, because of low coverage, this study focuses on first author and their affiliations.
