...
Previous studies have focused on specific countries [@P_l_nen_2020; @Huang_2020; @Haucap_2021; @Wenaas_2022; @Taubert_2023] or publisher portfolios [@Pieper_2018; @Bakker_2024; @Fraser_2023; @Jahn_2021; @Momeni_2023; @Schmal_2024], while large-scale studies relied on self-reported agreement data [@Moskovkin_2022], or used APC pricing lists [@Shu_2023]. 
...
It also includes code used to compile the data by connecting it to the cloud-based Google Big Query data warehouse, where scholarly big data from Crossref, OpenAlex and Unpaywall were imported, using bigrquery [@bigrquery].
...
Overall, `r n_tas_with_oa` transformative agreements recorded at least one open access article from an eligible institution.
...
country_scatter_22 <- country_scatter_df |>
  ...
  geom_smooth(aes(all, oa / all), method = lm) +
...
Particularly, Scandinavian countries, the Netherlands and Austria recorded above-average open access shares, as indicated by the linear trend line.
...
In line with previous research findings [@Butler_2023; @Jahn_2021; @Wenaas_2022], high uptake rates were observed across hybrid journals in  Social Sciences and Humanities.
