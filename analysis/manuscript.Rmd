...
For instance, the UK's Joint Information Systems Committee (JISC) concluded from an evaluation of its transformative agreements [@Jisc_2024] that while these agreements had a significant impact on the growth of open access in the UK, they had limited effect in facilitating a global shift towards full open access. 
Consequently, the report argues for a reassessment of the UK's open access strategy, in which hybrid OA and transformative agreements have played an important role.
After making similar observation, Norwegian and Swedish universities and their consortia also revised their policies [@norway; @Widding_2024]. 
As a result, the Association of Swedish Higher Education Institutions (Sveriges universitets- och högskoleförbund, SUHF) recommended only supporting agreements for publishing in fully open access journals by 2026 [@Widding_2024].
Likewise, cOAlition S will stop supporting transformative agreements at the end of 2024 [^coalitions].
...
Likewise, article-level open access invoicing and cost data are only partly available [@Jahn_2021; @Kramer_2024].
...
Because the corresponding authorship field was not fully supported by OpenAlex at the time of analysis, and affiliations were only available for 54% of the investigated articles, this study focused on first authors and their affiliations instead; approximately 90% of the investigated articles had first author affiliation metadata in OpenAlex, which is a much larger proportion than previously reported for the October 2022 snapshot [@Zhang_2024].
First authors typically contribute the most to a paper and are often considered lead authors [@Larivi_re_2016].
Examining Web of Science and Scopus, @Chinchilla_Rodr_guez_2024 revealed that the corresponding authors were typically the first authors listed. 
However, notable differences were observed between the two databases.
Related studies also utilised first authors as a proxy to examine open access payments and transformative agreements [@Zhang_2022; @Shu_2023; @Haucap_2021].
...
country_scatter_22 <- country_scatter_df |>
  ...
  geom_smooth(aes(all, oa / all), method = "lm", se = FALSE) +
...
Overall, the methodology is designed to underestimate, rather than overestimate, the adoption of open access through transformative agreements. 
